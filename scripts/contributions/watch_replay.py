...
class Unit:
	...
	def set_color(self):
		...
		if self.stability > MAX_HP[self.unit_type]:
			# debug log: patches[1] -> patches[0]
			self.patches[0].set_fill(False)
			self.patches[0].set_alpha(0.5)
...
class Graph:
	...
	def setup_board(self):
		...
		# debug log: range(29) required to match number of ticks
	...
	def cache_units(self, units, p_index):
		# debug log: the old script ignores upgrades and cannot correctly unpack values
